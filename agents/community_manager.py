...
MODEL = model_for("community-manager")
...
class CommunityManager:
    ...
    def draft_replies(self, comments: list[dict]) -> dict:
        ...
        # Build an index so the model references comments by comment_id (which it can't forge).
...
For EACH comment, do this:
1. Categorize it as exactly one of: purchase_intent, question, positive, negative, spam, prospect.
...
2. If the category is one of [purchase_intent, question, positive, negative, prospect], write THREE
   distinct reply variants in Gaurav's voice, then pick the winner and say why. For `negative`,
   de-escalate and be gracious — never defensive. Keep replies short and human (1–3 sentences),
   no hashtags, no emoji spam, no "Thanks for reaching out!" boilerplate.
3. If the category is spam, set action="ignore" and write no replies.
...
{{
  "drafts": [
    {{
      ...
      "category": "purchase_intent|question|positive|negative|spam|prospect",
      "action": "reply|ignore",
...
Every comment_id in the input MUST appear exactly once in drafts."""
        ...
        self.log(f"Drafting replies for {len(comments)} comment(s) via {MODEL}...")
        ...
        try:
            ...
            if idx >= 0:
                try:
                    ...
                    pass
            raise ValueError(f"Could not parse draft response: {raw[:200]}")
